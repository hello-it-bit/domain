...
def add_highfreq_perturb(noisy_in: torch.Tensor, c_latent: int,
    ...
    """
    noisy_in: [B, T, C_total, H, W]
    只对前 c_latent 通道做高频扰动
    """
    if strength <= 0:
        return noisy_in
    ...
    k = k.repeat(C, 1, 1, 1)             # depthwise: groups=C
    hi = F.conv2d(y, k, padding=1, groups=C)

    # 回到原 shape
    hi = hi.view(shape)                   # [B, T, C, H, W]

    # 每通道标准化（在 H,W 上），保持数值稳
    # 用空间维（H、W）求标准差，并保留两维，得到 [B, T, C, 1, 1]
