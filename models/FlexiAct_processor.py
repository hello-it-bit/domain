...
#     """
#     对 content_feat 进行自适应实例归一化，使其统计特征（均值和方差）匹配 style_feat。
#     Args:
#         content_feat (torch.Tensor): 需要被归一化的内容特征，即 FAE embedding。
#                                      形状: [B, N, C] B=批次, N=token数, C=特征维度
#         style_feat (torch.Tensor):   提供目标统计特征的风格特征，即 文本 tokens。
#                                      形状: [B, M, C] M=token数
#     Returns:
#         torch.Tensor: 归一化后的内容特征。
#     """
#     # 保持原始形状以便后续恢复
#     orig_shape = content_feat.shape
#     # 为了计算统计量，将 B 和 N 合并
#     content_feat = content_feat.view(orig_shape[0], -1, orig_shape[-1])
#
#     # 计算文本 tokens 的均值和标准差作为目标分布
#     # 在序列长度和批次维度上计算，保留特征维度
#     style_mean = torch.mean(style_feat, dim=[0, 1], keepdim=True)
#     style_std = torch.std(style_feat, dim=[0, 1], keepdim=True)
#
#     # 计算 FAE embedding 的均值和标准差
#     content_mean = torch.mean(content_feat, dim=[0, 1], keepdim=True)
#     content_std = torch.std(content_feat, dim=[0, 1], keepdim=True)
#
#     # AdaIN 核心公式
#     normalized_feat = (content_feat - content_mean) / (content_std + 1e-6)
#     new_feat = normalized_feat * style_std + style_mean
#
#     # 恢复原始形状
#     return new_feat.view(orig_shape)
#
#
...
#
#             self.motion_inversion_tokens = nn.Parameter(torch.empty(1, num_motion_tokens, 3072))   # we now change initial
#             nn.init.normal_(self.motion_inversion_tokens, std=0.02)  # 使用小的标准差进行正态分布初始化
#
#
...
#
#             # +++++++++++++++ 3. 在这里添加AdaIN调用 +++++++++++++++
#             # 动态地将 FAE embedding 的分布对齐到文本 tokens 的分布
#             aligned_cat_tokens = adaptive_instance_normalization(cat_tokens, encoder_hidden_states)
#             # +++++++++++++++++++++++++++++++++++++++++++++++++++++
#
#             # 使用对齐后的 tokens进行拼接
#             encoder_hidden_states = torch.cat([encoder_hidden_states, aligned_cat_tokens], dim=1)
#             encoder_h_seq_length = encoder_hidden_states.size(1)
#             hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)
#
#         else:
...
class RefNetLoRAProcessor(nn.Module):
    ...
    def __init__(
        ...
        if stage is not None:
            ...
            self.motion_inversion_tokens = nn.Parameter(torch.zeros(1, num_motion_tokens, 3072))
            nn.init.zeros_(self.motion_inversion_tokens)
            print(f"[FAE-INIT] motion_inversion_tokens shape = {self.motion_inversion_tokens.shape}, "
                  f"num_tokens = {self.num_motion_tokens}, emb_dim = 3072, total_params = {self.motion_inversion_tokens.numel()}")
    ...
    def save_attn_map(self, q_vis, k_vis, v_vis, save_step=[10], save_layer=[40]):
        ...
        if cur_step in save_step and cur_layer in save_layer:
            ...
            for i in range(q_vis.shape[1]):
                q_mini = q_vis[:, i:i + 1]
                k_mini = k_vis[:, i:i + 1]
                attn_map_mini = F.scaled_dot_product_attention(q_mini, k_mini, v_vis, attn_mask=None, dropout_p=0.0,
                                                               is_causal=False)  # [heads, seq_len, seq_len]
                attn_map += attn_map_mini
            save_path = os.path.join(self.attn_map_save_path, f"{cur_step}_{cur_layer}.pt")
            torch.save(attn_map, save_path)
