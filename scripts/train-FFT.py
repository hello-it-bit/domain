...
def _ring_mask(H, W, r1, r2, device, soft=True):
    ...
    if not soft:
        ...
        dr = max(1e-6, 0.04)       # 软边宽度，减振铃
        m1 = (rr - (r1 - dr)) / dr
        m2 = ((r2 + dr) - rr) / dr
        m  = torch.clamp(m1, 0, 1) * torch.clamp(m2, 0, 1)
        m  = 0.5 - 0.5*torch.cos(m * torch.pi)
        m  = m * ((rr >= (r1-dr)) & (rr <= (r2+dr))).float()
    return m

def add_highfreq_perturb(
    noisy_in, c_latent, strength=0.1, jitter=0.1, *,
    ring_r1=0.25, ring_r2=0.45
):
    if strength <= 0:
        return noisy_in
    ...
    # FFT
    Y   = tfft.fftn(y, dim=(-2,-1))
    mag = Y.abs()
    ph  = torch.angle(Y)   # 保留原相位

    # 环带掩码（软边减少振铃）
    mask = _ring_mask(H, W, ring_r1, ring_r2, y.device, soft=True)
    while mask.dim() < Y.dim():
        mask = mask.unsqueeze(0)  # -> [1,1,H,W]

    # 仅幅度增益（相位不变）
    mag2 = mag * (1.0 + 1.0 * mask)  # 轻增益；真正强度由时域 strength 控制
                ...
                with torch.no_grad():
                    ...
                    if use_hf:
                        ...
                        noisy_model_input = add_highfreq_perturb(
                            noisy_model_input, c_latent=c,
                            strength=float(getattr(args, "hf_strength", 0.1)),
                            jitter=float(getattr(args, "hf_jitter", 0.1)),
                            ring_r1=float(getattr(args, "hf_ring_r1", 0.25)),
                            ring_r2=float(getattr(args, "hf_ring_r2", 0.45)),
                        )
