...
from models.FlexiAct_processor1 import FeRARefNetLoRAProcessor, FeRARoutingState
import itertools
...
def main(args):
    ...
    router_tau=0.7,
    router_hidden=64,
    detach_alpha=False,).to(accelerator.device)
    ...
    # =========================
    # FeRA router load & freeze
    # =========================
    def _load_router_from_dir(router_dir: str):
        router_path = os.path.join(router_dir, "router.pt")
        if os.path.exists(router_path):
            routing_state.router.load_state_dict(torch.load(router_path, map_location="cpu"))

    # Resume/load router weights
    if args.stage is None:
        # RefAdapter training: router must be restored when resuming
        ...
        # FAE training: load router from RefAdapter checkpoint for completeness, but freeze it
        if args.resume_from_checkpoint is not None and path is not None:
            _load_router_from_dir(os.path.join(args.output_dir, path))
        elif args.ckpt_path is not None:
            _load_router_from_dir(os.path.dirname(args.ckpt_path))
    ...
    # Ensure FeRA router is trainable only in RefAdapter stage
    if args.stage is None:
        for p in routing_state.router.parameters():
    ...
    # Build optimizer param groups (deduplicate shared parameters for DeepSpeed/torch.optim)
    ...
    # IMPORTANT:
    # routing_state is shared across many attention processors; its parameters can appear multiple times
    # when iterating transformer.parameters(). We deduplicate above, and we DO NOT add router params
    # as a separate param group (to avoid 'parameters appear in more than one group').
    ...
    if args.stage is None:
        ...
        # remove overlap with transformer trainable params
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                latent2d = noisy_video.mean(dim=1)                # [B, C, H, W]

                if args.stage is None:
                    routing_state.update_from_latent(latent2d)
                else:
                    routing_state.clear()
                
                if args.stage is None and accelerator.is_main_process and global_step % 200 == 0 and routing_state.alpha is not None:
                    with torch.no_grad():
                        ...
                        # 3) FEI 能量分布（如果你在 routing_state 里存了 e_t）
                        if hasattr(routing_state, "e_t"):
                            fei_mean = routing_state.e_t.mean(dim=0)
                        else:
                            fei_mean = None

                        print(
                            f"[Step {global_step}] "
                            f"alpha_mean={alpha_mean.tolist()} | "
                            f"entropy={entropy.item():.4f}"
                            + (f" | fei_mean={fei_mean.tolist()}" if fei_mean is not None else "")
                        )
