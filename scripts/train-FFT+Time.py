...
def _ring_mask(H, W, r1, r2, device):
    fy = torch.fft.fftfreq(H, d=1.0).to(device).view(H,1).expand(H,W)
    fx = torch.fft.fftfreq(W, d=1.0).to(device).view(1,W).expand(H,W)
    rr = torch.sqrt(fx*fx + fy*fy)  # 0~0.5
    m = ((rr >= r1) & (rr <= r2)).float()
    return m  # [H,W]

def _motion_map_from_latents(z_btchw):
    # 简易运动边界图（帧差+梯度），返回 [B,T,1,H,W] ∈ [0,1]
    B,T,C,H,W = z_btchw.shape
    if T < 2:
        return torch.zeros(B, T, 1, H, W, device=z_btchw.device, dtype=z_btchw.dtype)
    delta = (z_btchw[:,1:] - z_btchw[:,:-1]).mean(dim=2, keepdim=True)  # [B,T-1,1,H,W]
    gx = F.pad(delta[..., 1:, :] - delta[..., :-1, :], (0,0,0,0,1,0))
    gy = F.pad(delta[..., :, 1:] - delta[..., :, :-1], (0,0,1,0,0,0))
    m = (gx.abs() + gy.abs())
    m = F.pad(m, (0,0,0,0,0,1))  # pad 回 T
    m = m / (m.flatten(3).amax(dim=3, keepdim=True).clamp_min(1e-6))
    return m.clamp(0,1)

def add_highfreq_perturb(
    noisy_in: torch.Tensor,       # [B,T,C_total,H,W]
    c_latent: int,                # 仅作用前 c_latent 通道
    strength: float = 0.1,        # 基础强度 α0
    jitter: float = 0.1,          # 乘性抖动
    *,
    timesteps=None,               # [B] 或标量；可为 None
    image_latents_for_motion: torch.Tensor = None,  # [B,T,C,H,W]，用于运动感知
):
    """
    频域环带高通（FFT Ring High-Pass）+ 相位微扰 + 时间步/内容自适应。
    插入点：拼接后、RoPE 前。保持张量形状不变，仅修改前 c_latent 通道。
    """
    if strength <= 0:
        return noisy_in

    x = noisy_in[:, :, :c_latent, :, :]           # 只改噪声潜变量通道
    B,T,C,H,W = x.shape
    y = x.contiguous().view(B*T, C, H, W).float()

    # === 1) 频域表示 ===
    Y = tfft.fftn(y, dim=(-2,-1))                 # [B*T, C, H, W] complex
    mag, ph = Y.abs(), torch.angle(Y)

    # 环带参数（可从 args 读取；未注册也有默认值）
    r1 = float(getattr(globals().get("args", {}), "hf_ring_r1", 0.25))  # 若外层未传 args，则用默认
    r2 = float(getattr(globals().get("args", {}), "hf_ring_r2", 0.45))
    phase_sigma = float(getattr(globals().get("args", {}), "hf_phase_sigma", 0.0))
    mask = _ring_mask(H, W, r1, r2, y.device)     # [H,W]
    while mask.dim() < Y.dim():
        mask = mask.unsqueeze(0)                   # -> [1,1,H,W] 广播

    # === 2) 内容自适应 g(b,t,h,w)：环带能量 + 边缘 + 运动 ===
    # 2.1 环带能量比（局部）: 提升有意义的高频区域
    ring_energy = (mag * mask).pow(2)
    rel = ring_energy / (ring_energy.mean(dim=(-2,-1), keepdim=True).clamp_min(1e-6))
    rel = torch.sqrt(rel).clamp(0.5, 2.0)         # 压缩动态范围

    # 2.2 边缘图（Scharr 近似）
    kx = torch.tensor([[3,0,-3],[10,0,-10],[3,0,-3]], device=y.device, dtype=y.dtype).view(1,1,3,3).repeat(C,1,1,1)
    ky = torch.tensor([[3,10,3],[0,0,0],[-3,-10,-3]], device=y.device, dtype=y.dtype).view(1,1,3,3).repeat(C,1,1,1)
    gx = F.conv2d(y, kx, padding=1, groups=C)
    gy = F.conv2d(y, ky, padding=1, groups=C)
    edge = (gx.abs() + gy.abs()).view(B,T,C,H,W)
    edge = edge / (edge.flatten(3).amax(dim=3, keepdim=True).clamp_min(1e-6))

    # 2.3 运动边界图（可选）
    mot = _motion_map_from_latents(image_latents_for_motion) if image_latents_for_motion is not None \
          else torch.zeros(B, T, 1, H, W, device=x.device, dtype=x.dtype)

    # 融合（乘性）：强调三者共同高的区域；给定温和权重
    w_edge = 0.5
    w_mot  = 0.3
    g = rel * (1.0 + w_edge * edge) * (1.0 + w_mot * mot)   # [B,T,C,H,W]（mot 会广播到 C）
    g = g.clamp(0.7, 1.8)

    # === 3) 时间步自适应 s(t) ===
    sched_gamma = float(getattr(globals().get("args", {}), "hf_sched_gamma", 0.0))  # 0=关闭调度
    if sched_gamma > 0.0 and timesteps is not None:
        total = getattr(timesteps, "num_train_timesteps", 1000.0)
        if isinstance(timesteps, torch.Tensor):
            t_norm = timesteps.float() / float(total)
            while t_norm.dim() < x.dim():
                t_norm = t_norm.view(-1, *([1]*(x.dim()-1)))  # [B,1,1,1,1]
        else:
            t_norm = torch.tensor(float(timesteps)/float(total), device=x.device, dtype=x.dtype).view(1,1,1,1,1)
        s_t = t_norm ** sched_gamma
    else:
        s_t = 1.0

    # === 4) 在环带内做幅度/相位操作（幅度按 g*s_t 提升；相位小扰动可选） ===
    boost = (1.0 + g * s_t) * mask + (1.0 - mask)   # 仅环带内提升
    mag2  = mag * boost
    if phase_sigma > 0:
        ph = ph + (torch.randn_like(ph) * phase_sigma) * mask

    Y2 = mag2 * torch.exp(1j * ph)
    hi = tfft.ifftn(Y2 - Y, dim=(-2,-1)).real.view(B,T,C,H,W)

    # === 5) 逐通道空间归一化 + 抖动 + 加回 ===
    std = hi.std(dim=(-2,-1), keepdim=True).clamp_min(1e-6)   # [B,T,C,1,1]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                with torch.no_grad():
                    ...
                    # === Add high-frequency perturbation (minimal & safe) ===
                    use_hf = bool(getattr(args, "use_hf_perturb", False))
                    if use_hf:
                        # c 是你原始潜变量通道数（上文已有 b,t,c,h,w = image_latents.shape）
                        ...
                        noisy_model_input = add_highfreq_perturb(
                            ...
                            timesteps=timesteps,  # 新增：时间步
                            image_latents_for_motion=image_latents,  # 新增：用于运动感知
                        )
