...
class VideoInpaintingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            io_open_ms = _ms(t1 - t0)
            io_decode_ms = _ms(t2 - t1)
            fps_down_ms = _ms(t3 - t2)
            ...
            sample_timing = {
                "io/open_ms": io_open_ms,
                "io/decode_ms": io_decode_ms,
                "io/fps_downsample_ms": fps_down_ms,
            }
...
class MyWebDataset():
    ...
    def __call__(self, examples):
        ...
        batch_timing = {
            "data/trim_frames_ms": 0.0,
            "data/np2torch_ms": 0.0,
            "data/resize_video_ms": 0.0,
            "data/resize_image_ms": 0.0,
            "data/normalize_ms": 0.0,
            "data/tokenize_ms": 0.0,
            "data/stack_ms": 0.0,
            # 汇总 __getitem__ 带来的 IO 时间
            "io/open_ms": 0.0,
            "io/decode_ms": 0.0,
            "io/fps_downsample_ms": 0.0,
        }

        max_num_frames = self.max_num_frames if self.is_train else self.val_max_num_frames
        for example in examples:

            if "timing" in example:
                for k, v in example["timing"].items():
                    batch_timing[k] = batch_timing.get(k, 0.0) + float(v)
            ...
            t1 = _now()
            batch_timing["data/trim_frames_ms"] += _ms(t1 - t0)

            # numpy -> torch; 选择 image
            t0 = _now()
            ...
            if self.use_different_first_frame:
                ...
                image = video # [1, C, H, W]
            t1 = _now()
            batch_timing["data/np2torch_ms"] += _ms(t1 - t0)
            t0 = _now()

            video = resize_for_rectangle_crop(video, self.resolution, reshape_mode="center")
            t1 = _now()
            batch_timing["data/resize_video_ms"] += _ms(t1 - t0)

            t0 = _now()
            # image = resize_wo_crop(image, self.resolution)
            image = resize_for_rectangle_crop(image, self.resolution, reshape_mode="center")
            t1 = _now()
            batch_timing["data/resize_image_ms"] += _ms(t1 - t0)
            ...
            # video = (video.astype(np.float32) / 127.5) - 1.0
            # image = (image.astype(np.float32) / 127.5) - 1.0
            video = video.to(dtype=torch.float32).div(127.5).sub(1.0)
            image = image.to(dtype=torch.float32).div(127.5).sub(1.0)
            if video.max() > 1.0 or video.min() < -1.0:
                raise ValueError("Image pixel values should be in the range [-1, 1]")

            t1 = _now()
            batch_timing["data/normalize_ms"] += _ms(t1 - t0)
            ...
            t1 = _now()
            batch_timing["data/tokenize_ms"] += _ms(t1 - t0)
        ...
        t1 = _now()
        batch_timing["data/stack_ms"] += _ms(t1 - t0)
